...
contract DigitalAssetSystem {
    ...
    function generateUniqueLicense() internal returns (uint256) {
        uint256 nonce = 0;
        uint256 license;
        bool licenseGenerated = false;

        while (!licenseGenerated) {
            uint256 randomNumber = uint256(keccak256(abi.encodePacked(block.timestamp, blockhash(block.number), msg.sender, nonce)));
            license = randomNumber % 10000000000;

            if (!usedLicenses[license]) {
